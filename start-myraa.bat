...
echo ============================================================
echo                 BELLA ALL-IN-ONE LAUNCHER
echo ============================================================
echo.
...
start "BELLA Desktop Agent" /MIN cmd /k "cd /d "%PROJECT_DIR%" && "%PYTHON_EXE%" -m uvicorn desktop_agent.main:app --host 127.0.0.1 --port 8765"
...
if "%READY%"=="0" (
    ...
    echo     BELLA will still run, but desktop control may be unavailable.
)
echo.

echo [4/4] Starting BELLA Server ^(Node, port 3000^)...
echo ============================================================
echo   Desktop Agent : http://127.0.0.1:8765
echo   BELLA UI      : http://localhost:3000
echo ============================================================
echo.
echo   Close this window to stop BELLA.
...
echo.
echo BELLA has stopped. Cleaning up Desktop Agent...
for /f "tokens=5" %%a in ('netstat -ano ^| findstr ":8765" ^| findstr "LISTENING"') do (
    taskkill /PID %%a /F >nul 2>&1
)
pause
